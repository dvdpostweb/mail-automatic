...
class exp_reply extends Script {
	...
	public function execute($mail_id)
	{
		...
		where s.email is not null and used_at is null and date(s.updated_at) = date(DATE_ADD(now(), INTERVAL -3 day)) and (name like "EXP%" or name like "TEXP%" or name like "SEXP%" or name like "FEXP%" or name like "WEXP%" or name like "PEXP%" ) ;';
